...
def main(
    ...
    input="Input 1",  # You can leave this empty
    # ... (rest of your function arguments
...
#    input: str = "",
 ...
 #  prompt = generate_prompt(sample)
    # Prepare the example and generate prompt
        # Prepare the example and generate prompt
    example = prepare_sample(
        example=dummy_example,
        tokenizer=None,  # Tokenization is not needed for user input
        max_length=None,  # Max length is not needed for user input
        mask_inputs=False,  # No need to mask user input
        ignore_index=None,  # No need to ignore index
    )
    prompt = generate_prompt(example["instruction"], "Input 1", "Input 2", "Input 3", ...)
